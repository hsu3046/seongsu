...
import { canWalk } from '../src/game/navigation.ts';
...
try {
  ...
  await page.waitForFunction((key) => JSON.parse(localStorage.getItem(key)).position.x < 780, key);
  ...
  await page.waitForFunction(({key, entrance}) => { const p = JSON.parse(localStorage.getItem(key)).position; return Math.hypot(p.x - entrance.x, p.y - entrance.y) < 4; }, {key, entrance: places[0].entrance});
  ...
  // From the arrival point, the café lies east. A held key must stop at its polygon.
  await page.keyboard.down('ArrowRight');
  await page.waitForTimeout(1000);
  await page.keyboard.up('ArrowRight');
  await page.getByRole('button', { name: 'How to explore', exact: true }).click();
  const collided = (await read()).position;
  assert.ok(canWalk(collided));
  assert.ok(collided.x < places[0].entrance.x + 70);
  await page.getByRole('button', { name: 'Back to the neighborhood', exact: true }).click();
